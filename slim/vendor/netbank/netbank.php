...
class NetBank{
	...
	public function __construct($clientNumber, $password){
		$this->initialData = $this->callNetBank(
			'login',
			array('UserName' => $clientNumber, 'Password' => $password, 'Token' => '')
		);
	...
	public function getAccountTransactions($id){
		$ret = $this->callNetBank('getTransactions', array('AccountID' => $id, 'AccountIdIsUser' => 'true'));
	 ...
	 * @returns object
	 */
	protected function callNetBank($req, $params = array(), $ignoreErrors = false){
		$url = self::netbankURL . '/mobile/i/AjaxCalls.aspx?SID=' . $this->SID;

		$params = array_merge(array('Request' => $req), $params);
